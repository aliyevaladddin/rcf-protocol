...
Correlation Framework (RCF) Protocol v2.0.4. Version 2.0.3, codenamed "Ghost Protocol", 
...
### 3.1 Ghost Marker Syntax (v2.0.4)
...
- **`[RCF:GHOST]`**: Active v2.0.4 protection. Requires valid signature to run.
...
To resist AI/ML logic extraction, RCF v2.0.4 injects "Semantic Noise" — code paths that are logically valid but semantically confusing to LLMs, without impacting runtime performance.
...
The `rcf-ghost-shield` tool replaces the legacy `rcf-cli` for v2.0.4 operations:
...
Projects migrating from v1.x to v2.0.4 MUST:
...
### Appendix A: RCF Marker Quick Reference (v2.0.4)
